...
export default function AddProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Build Payload
    // Backend API takes regular fields: name, slug, description, price, discountPrice, stock, sku, image, status, categoryId.
    const payload = {
      name: name.trim(),
      slug: slug.trim(),
    ...
    try {
      ...
      if (data.success) {
        setSuccessMsg('Product added successfully! Redirecting to products list...');
        // Clear form
        setName('');
        setSlug('');
        setDescription('');
        ...
        // Redirect to inventory page after a brief delay
        setTimeout(() => {
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
              ...
              <div className="border border-dashed border-zinc-200 rounded-2xl p-4 bg-zinc-50 flex items-center justify-center min-h-[160px] relative overflow-hidden">
                {image.trim() ? (
                  ...
                  <div className="flex flex-col items-center justify-center text-zinc-400 gap-1 text-center p-6">
                    ...
                    <span className="text-xs text-zinc-400">Enter a valid Unsplash or online image URL above to view preview</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm flex flex-col gap-4">
            ...
            <Link
              href="/admin/products"
              className="w-full rounded-full border border-zinc-200 bg-zinc-50 hover:bg-zinc-100 text-center py-3 text-sm font-semibold text-zinc-700 transition-all disabled:opacity-50 flex items-center justify-center"
            >
              Cancel
            </Link>
          </div>
